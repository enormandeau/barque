...
cd "$step"
for i in $(grep -v "^#" ../02_info/primers.csv | awk '{print $1}')
do
    echo -e "$step\t$step" > ../13_read_dropout/"$step"_"$i"
    for j in $(ls -1 *merged_"$i"*.gz | grep "$i")
    do
        echo -e $(echo "$j" | cut -d "_" -f 1)"\t"$(echo $(gunzip -c "$j" | wc -l) / 4 | bc | cut -d "." -f 1)
    done | sort -V >> ../13_read_dropout/"$step"_"$i"
done
cd ..
...
for i in $(grep -v "^#" ../02_info/primers.csv | awk '{print $1}')
do
    echo -e "$step\t$step" > ../13_read_dropout/"$step"_"$i"; awk 'NR==1 {for (i=1; i<=NF; i++) {headers[i]=$i}} NR>1 {for (i=1; i<=NF; i++) {sums[i]+=$i}} END {for (i=4; i<=NF; i++) {print headers[i]"\t"sums[i]}}' "$i"_species_results.csv | sort -V >> ../13_read_dropout/"$step"_"$i"
done
cd ..
