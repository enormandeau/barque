...
done | sort -g >> ../13_read_dropout/$step
cd ..
...
do
    echo -e $(echo "$i" | cut -d "_" -f 1)"\t"$(echo $(gunzip -c "$i" | wc -l) / 4 | bc | cut -d "." -f 1)
done | sort -g | awk '{print $2}' >> ../13_read_dropout/$step
cd ..
...
for i in $(grep -v "^#" ../02_info/primers.csv | awk -F "," '{print $1}')
do
    echo -e "$step" > ../13_read_dropout/"$step"_"$i"; awk -F "," 'NR==1 {for (i=1; i<=NF; i++) {headers[i]=$i}} NR>1 {for (i=1; i<=NF; i++) {sums[i]+=$i}} END {for (i=5; i<=NF; i++) {print headers[i]"\t"sums[i]}}' "$i"_species_table.csv | sort -g | awk '{print $2}' >> ../13_read_dropout/"$step"_"$i"
done
cd ..
...
cd "$step"
paste -d "," 04_data 05_trimmed 06_merged 07_split_amplicons_* 08_chimeras_* 12_summary* > ../15_results/sequence_dropout.csv
cd ..
