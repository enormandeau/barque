...
do
    ...
    do
        ...
        #vsearch -vsearch_local "$CHIMERAFOLDER"/"$fasta" -db 03_databases/"$database" -id "$SIMILARITY_VSEARCH" \
        #    -maxaccepts "$MAX_ACCEPTS" -maxrejects "$MAX_REJECTS" -strand both -blast6out \
        #    "$VSEARCHFOLDER"/"${fasta%.fasta}"."${database%.udb}" -top_hit_only -query_cov "$QUERY_COV"
